...
Camera_Controller::Camera_Controller(Urho3D::Context * context)
...
{
    SubscribeToEvent(E_UPDATE, URHO3D_HANDLER(Camera_Controller, handle_update));
    SubscribeToEvent(E_INPUT_TRIGGER, URHO3D_HANDLER(Camera_Controller, handle_input_event));
}

void Camera_Controller::release()
{
    UnsubscribeFromAllEvents();
}

void Camera_Controller::set_camera(Urho3D::Camera * cam)
{
    cam_comp_ = cam;
...
Urho3D::Camera * Camera_Controller::get_camera()
{
    return cam_comp_;
}

Camera_Params * Camera_Controller::get_camera_params()
{
    return &cp_;
}

void Camera_Controller::add_viewport(int vp_ind)
{
    if (!viewports_.Contains(vp_ind))
        viewports_.Push(vp_ind);
}

void Camera_Controller::remove_viewport(int vp_ind)
{
    viewports_.Remove(vp_ind);
}

void Camera_Controller::handle_update(Urho3D::StringHash event_type,
...
void Camera_Controller::setup_input_context(Input_Context * ctxt)
{
...
void Camera_Controller::set_orbit_point(const fvec3 & point)
{
    if (cam_comp_ == nullptr)
        return;
...
fvec3 Camera_Controller::get_orbit_point() const
{
    return orbit_node_->GetWorldPosition();
}

void Camera_Controller::_on_cam_pitch_yaw(const Urho3D::Vector2 & mdelta)
{
    if (cam_comp_ == nullptr)
        return;
    Urho3D::Node * cam_node = cam_comp_->GetNode();
...
void Camera_Controller::_on_cam_orbit_set_point(const Urho3D::Vector2 & norm_mpos, Urho3D::Viewport * vp)
{
    if (cam_comp_ == nullptr)
        return;
...
void Camera_Controller::_on_cam_orbit(const Urho3D::Vector2 & mdelta)
{
    if (cam_comp_ == nullptr)
        return;
    Urho3D::Node * cam_node = cam_comp_->GetNode();
...
void Camera_Controller::_on_cam_move(const Urho3D::Vector2 & mdelta)
{
    if (cam_comp_ == nullptr)
        return;
    Urho3D::Node * cam_node = cam_comp_->GetNode();
...
void Camera_Controller::_on_cam_zoom(int wheel)
{
    Force_Timer ref;
...
void Camera_Controller::handle_input_event(Urho3D::StringHash event_type,
                                                  Urho3D::VariantMap & event_data)
{
    if (cam_comp_ == nullptr)
        return;
    ...
    if (!viewp)
    {
        dout << "GOT BAD VP POINTER FOR CAM!";
        return;
    }
    ...
    {
        _on_cam_orbit_set_point(norm_mpos, viewp);
    }
