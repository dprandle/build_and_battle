        ...
        while (tr.first != tr.second)
        {
                ...
                pass_qual_allowed && pass_mb_allowed)
            {
                if ((trig->trigger_state_ & T_BEGIN) == T_BEGIN)
                {
                    ...
                    _fill_vp_info(vp_info, current_norm_mpos_, fvec2());
                    if (vp_info.vp)
                    {
                        event_data[P_VIEWPORT_NORM_MPOS] = vp_info.vp_norm_mpos;
                        event_data[P_VIEWPORT_NORM_MDELTA] = vp_info.vp_norm_mdelta;
                        event_data[P_VIEWPORT_PTR] = vp_info.vp;
...
void Input_Translator::push_viewport(const Viewport_Data & vp)
{
    vp_stack_.Push(vp);
}

void Input_Translator::pop_viewport()
{
    vp_stack_.Pop();
}

void Input_Translator::_fill_vp_info(Viewport_Info & vp_inf,
                                     ...
                                     const fvec2 &norm_mdelta)
{
    ...
    for (int i = 0; i < vp_stack_.Size(); ++i)
    {
        Viewport* vp = vp_stack_[i].vp;
        UIElement* ui_elem = vp_stack_[i].view_element;

        irect scrn = vp->GetView()->GetViewRect();
        ivec2 min = scrn.Min();
        ivec2 max = scrn.Max();

        if (ui_elem)
        {
            min = ui_elem->GetScreenPosition();
            max = max + min;
        }
        
        fvec2 ll(min.x_,min.y_);
        fvec2 ur(max.x_, max.y_);
        ...
        {
            vp_inf.vp = vp;
            vp_inf.vp_norm_mpos = (mpos - ll) / fr.Size();
            vp_inf.vp_norm_mdelta = mdelta / fr.Size();
            return;
        }
    }
}
        ...
        while (tr.first != tr.second)
        {
                ...
                pass_qual_allowed && pass_mb_allowed)
            {
                if ((trig->trigger_state_ & T_BEGIN) == T_BEGIN)
                {
                    ...
                    // Send an event to each viewport
                    Viewport_Info vp_info;
                    _fill_vp_info(vp_info, current_norm_mpos_, norm_mdelta);
                    if (vp_info.vp)
                    {
                        event_data[P_VIEWPORT_NORM_MPOS] = vp_info.vp_norm_mpos;
                        event_data[P_VIEWPORT_NORM_MDELTA] = vp_info.vp_norm_mdelta;
